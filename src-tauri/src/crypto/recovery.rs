...
use crate::utils::{
    config::{AES_KEY_SIZE, RECOVERY_SEGMENTS, RECOVERY_SEGMENT_LEN},
...
/// Example: `A3F2-9C81-B742-DE04-5F1A`
#[derive(Debug, Clone)]
pub struct RecoveryKey {
    /// The raw 32-byte entropy backing the recovery key.
    entropy: Zeroizing<[u8; AES_KEY_SIZE]>,
    /// Display string in `XXXX-XXXX-XXXX-XXXX-XXXX` format.
    pub display: String,
}
...
impl RecoveryKey {
    ...
    pub fn generate() -> VaultisResult<Self> {
        let bytes = random::random_bytes(AES_KEY_SIZE)?;
        let mut entropy = Zeroizing::new([0u8; AES_KEY_SIZE]);
    ...
    pub fn from_str(s: &str) -> VaultisResult<Self> {
        ...
        let expected_len = RECOVERY_SEGMENTS * RECOVERY_SEGMENT_LEN;
    ...
    pub fn raw(&self) -> &[u8; AES_KEY_SIZE] {
        &self.entropy
    }
}

/// Format 32 bytes as `XXXX-XXXX-XXXX-XXXX-XXXX` uppercase hex groups.
fn format_recovery_key(entropy: &[u8; AES_KEY_SIZE]) -> String {
    let hex_str = hex::encode_upper(entropy.as_slice());
    // Take first RECOVERY_SEGMENTS * RECOVERY_SEGMENT_LEN chars = 20 chars
    let trimmed: String = hex_str.chars().take(RECOVERY_SEGMENTS * RECOVERY_SEGMENT_LEN).collect();
    trimmed
        .chars()
        .collect::<Vec<char>>()
